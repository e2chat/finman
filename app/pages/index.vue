  ...
  <div class="min-h-screen bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
    ...
    <header class="sticky top-0 z-10 bg-white/80 dark:bg-neutral-900/80 backdrop-blur border-b border-neutral-200 dark:border-neutral-700">
      <div class="mx-auto max-w-4xl px-4 py-3 flex items-center gap-3">
        ...
        <button @click="goAdd" class="ml-2 inline-flex items-center justify-center w-9 h-9 rounded-full bg-neutral-900 dark:bg-neutral-100 text-white dark:text-neutral-900 hover:bg-neutral-800 dark:hover:bg-neutral-200">+</button>
      </div>
    </header>
    ...
    <main class="mx-auto max-w-4xl px-4 py-6">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div v-for="i in filtered" :key="i.id" @click="openItem(i.id)" class="cursor-pointer rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-4 shadow-sm hover:shadow transition">
          <div class="flex items-start justify-between">
            <div class="font-medium text-neutral-800 dark:text-neutral-200">{{ typeLabel[i.type] }}</div>
            <div class="text-sm text-neutral-500 dark:text-neutral-400">{{ pct(i) }}%</div>
          </div>
          ...
          <div class="mt-3 h-4 w-full rounded-full bg-neutral-200 dark:bg-neutral-700 overflow-hidden">
            <div class="h-full bg-neutral-800 dark:bg-neutral-300" :style="{ width: pct(i) + '%' }"></div>
          </div>
