...
function applyPercentageToTarget(percentage: number) {
  ...
  if (!i) return;
  if (percentage > 50) {
    if (!confirm(`This will increase your target amount by ${percentage}%. Are you sure?`)) return;
  }
  ...
  <div class="min-h-screen bg-neutral-50 dark:bg-neutral-900">
    ...
    <main class="mx-auto max-w-2xl px-4 py-6" v-if="item">
      ...
      <div class="mt-6 rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-4 shadow-sm space-y-3">
        ...
        <p class="text-xs text-neutral-500 dark:text-neutral-400">Apply one-time percentage increase to target amount (e.g., interest)</p>
        ...
        <div class="flex items-center gap-2">
          <input v-model.number="customPercentage" type="number" min="0" step="0.1" class="flex-1 rounded-md border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-700 text-neutral-900 dark:text-neutral-100 placeholder:text-neutral-500 dark:placeholder:text-neutral-400 px-3 py-2 text-sm" placeholder="Custom %" />
